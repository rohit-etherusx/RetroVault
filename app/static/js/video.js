class GameBoyAdvanceVideo {
	constructor() {
		// Force software renderer to avoid worker-related issues during debugging
		this.renderPath = new GameBoyAdvanceSoftwareRenderer();
		console.log('Using software renderer (forced)');
	...
	finishDraw(pixelData) {
		if (!window.__rv_first_frame_reported) {
			window.__rv_first_frame_reported = true;
			try {
				fetch('/client-log', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({event: 'first_frame_drawn'})}).catch(()=>{});
			} catch (_) {}
		}
