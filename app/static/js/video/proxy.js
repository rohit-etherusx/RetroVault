...
class GameBoyAdvanceRenderProxy {
	constructor() {
		...
		// Notify server that worker was instantiated (diagnostic)
		try {
			fetch('/client-log', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({event: 'worker_init', url: '/static/js/video/worker.js'})}).catch(()=>{});
		} catch(_) {}

		// Surface worker errors to the main console for logging
		this.worker.onerror = function(err) {
			console.error('Video worker error:', err && (err.message || err));
			try { fetch('/client-log', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({event: 'worker_error', message: (err && err.message) || String(err)})}).catch(()=>{}); } catch(_) {}
		};
